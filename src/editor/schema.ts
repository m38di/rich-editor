...
export type MediaKind = 'image' | 'video' | 'animation' | 'audio' | 'document'
...
/**
 * TL_keyboard.RichButtonStyle → DEFAULT/PRIMARY/DANGER/SUCCESS pills, plus
 * `link`: a callback_data-only style that renders as a plain INLINE LINK
 * (accent + underline) instead of a pill.
 */
export type ButtonStyle = 'default' | 'primary' | 'danger' | 'success' | 'link'

/** Official <tg-button type="…"> values (Bot API rich-HTML style). */
export const BUTTON_TYPES = [
  'url', // payload in `url`
  'callback_data', // payload in `data`
  'web_app', // payload in `url` — private chats only
  'login_url', // payload in `url`, optional forward-text / request-write-access
  'switch_inline_query', // payload in `query`
  'switch_inline_query_current_chat',
  'switch_inline_query_chosen_chat', // optional allow-* chat filters
  'copy_text', // payload in `text`
  'disabled',
] as const
export type ButtonType = (typeof BUTTON_TYPES)[number]

/** Per-type parameter payload of a tg-button, matching the official attrs. */
export interface ButtonParams {
  url?: string // url / web_app / login_url
  data?: string // callback_data
  query?: string // switch_inline_query*
  text?: string // copy_text
  forwardText?: string // login_url
  requestWriteAccess?: boolean // login_url
  allowUserChats?: boolean // switch_inline_query_chosen_chat
  allowBotChats?: boolean
  allowGroupChats?: boolean
  allowChannelChats?: boolean
}

/** One pill in a button_row (Android RichButtonRowCell, MAX_BUTTONS = 8). */
...
// ── import helpers (HTML → node attrs) ──────────────────────────────────
// These back the parseDOM rules below so that files produced by the
// Download step (and reasonable arbitrary HTML) round-trip into the editor.
...
function kindOf(el: HTMLElement): MediaKind {
  switch (el.tagName.toLowerCase()) {
    ...
    case 'tg-document': return 'document'
    default: return 'image'
  }
}

/** <img>/<video>/<audio>/<tg-document> → media attrs (src may sit on <source>). */
function mediaAttrs(el: HTMLElement, kind: MediaKind) {
  const source = el.querySelector('source')
  return {
    kind,
    src: el.getAttribute('src') || (source?.getAttribute('src') ?? ''),
    spoiler: el.hasAttribute('tg-spoiler'),
    title: el.getAttribute('title') || el.getAttribute('data-title') || '',
    author: el.getAttribute('data-author') || '',
  }
}

/** Children of a gallery element → MediaItem[] (skips the shared caption). */
function groupAttrs(dom: HTMLElement, mode: GalleryMode) {
  const items: MediaItem[] = []
  dom.querySelectorAll('img, video, audio, tg-document').forEach((m) => {
    const el = m as HTMLElement
    const src = el.getAttribute('src') || el.querySelector('source')?.getAttribute('src') || ''
    if (!src) return
    items.push({ kind: kindOf(el), src, spoiler: el.hasAttribute('tg-spoiler') })
  })
  return { mode, items }
}

function mapAttrs(el: HTMLElement) {
  return {
    lat: Number(el.getAttribute('lat')) || 41.9,
    long: Number(el.getAttribute('long')) || 12.5,
    zoom: Number(el.getAttribute('zoom')) || 15,
  }
}

/**
 * Content element for <figure>-style nodes: the media itself becomes attrs,
 * so only the caption (if any) is parsed as node content. Returns a wrapper
 * holding a figcaption clone — its children then parse into `fig_caption?`.
 */
function captionContent(dom: HTMLElement): HTMLElement {
  const wrap = dom.ownerDocument.createElement('div')
  const cap = dom.querySelector('figcaption')
  if (cap) wrap.appendChild(cap.cloneNode(true))
  return wrap
}
...
const nodes: Record<string, NodeSpec> = {
  ...
  list_item: {
    ...
    // context-scoped: inside task_list the task_item rule must win
    // (rules are picked by tag + context only, not by content fit)
    parseDOM: [{ tag: 'li', context: 'bullet_list/|ordered_list/' }],
    toDOM: () => ['li', 0],
  },
  ...
  media_figure: {
    attrs: {
      kind: { default: 'image' }, // MediaKind (incl. document)
      src: { default: '' },
      spoiler: { default: false },
      title: { default: '' }, // document name / audio title
      author: { default: '' }, // e.g. file size for documents, artist for audio
    },
    content: 'fig_caption?',
    group: 'block',
    isolating: true,
    parseDOM: [
      // official rich-HTML form: <tg-document src="…"></tg-document>
      {
        tag: 'tg-document',
        getAttrs: (el) => {
          const dom = el as HTMLElement
      ...
      // bare media elements (import): <img>, <video>, <audio>
      { tag: 'img', getAttrs: (el) => mediaAttrs(el as HTMLElement, 'image') },
      { tag: 'video', getAttrs: (el) => mediaAttrs(el as HTMLElement, 'video') },
      { tag: 'audio', getAttrs: (el) => mediaAttrs(el as HTMLElement, 'audio') },
      // <figure> wrapping a single media element + optional caption.
      // Gallery/map figures are rejected here so their own rules (higher
      // priority) claim them.
      {
        tag: 'figure',
        getAttrs: (el) => {
          const dom = el as HTMLElement
          if (dom.querySelector('tg-map, tg-collage, tg-slideshow')) return false
          const media = dom.querySelector('img, video, audio, tg-document')
          if (!media) return false
          return mediaAttrs(media as HTMLElement, kindOf(media as HTMLElement))
        },
        contentElement: captionContent,
      },
    ],
    toDOM: () => ['figure', 0],
  },
  ...
  media_group: {
    ...
    parseDOM: [
      {
        tag: 'tg-collage',
        getAttrs: (el) => groupAttrs(el as HTMLElement, 'collage'),
        contentElement: captionContent,
      },
      {
        tag: 'tg-slideshow',
        getAttrs: (el) => groupAttrs(el as HTMLElement, 'slideshow'),
        contentElement: captionContent,
      },
      // <figure> wrapping a gallery + optional caption (export form)
      {
        tag: 'figure',
        priority: 75,
        getAttrs: (el) => {
          const dom = el as HTMLElement
          const group = dom.querySelector('tg-collage, tg-slideshow')
          if (!group) return false
          return groupAttrs(
            group as HTMLElement,
            group.tagName === 'TG-SLIDESHOW' ? 'slideshow' : 'collage',
          )
        },
        contentElement: captionContent,
      },
  ...
  map_block: {
    ...
    parseDOM: [
      {
        tag: 'tg-map',
        getAttrs: (el) => mapAttrs(el as HTMLElement),
      },
      // <figure> wrapping a map + optional caption (export form)
      {
        tag: 'figure',
        priority: 80,
        getAttrs: (el) => {
          const dom = el as HTMLElement
          const map = dom.querySelector('tg-map')
          return map ? mapAttrs(map as HTMLElement) : false
        },
        contentElement: captionContent,
      },
    ],
  ...
  // Row of pill buttons (Android pageBlockButtonRow / RichButtonRowCell,
  // MAX_BUTTONS = 8; tap on a button cycles its style). Parses the official
  // <tg-button-row><tg-button type="url" style="…" url="…">Label</tg-button></tg-button-row>.
  button_row: {
    attrs: {
      buttons: { default: [] }, // RowButton[]
    },
    group: 'block',
    atom: true,
    isolating: true,
    ...
    parseDOM: [
      {
        ...
        getAttrs: (el) => {
          const dom = el as HTMLElement
          const styles = ['default', 'primary', 'danger', 'success']
          const buttons = Array.from(dom.querySelectorAll('tg-button')).map((b) => {
            const raw = (b.getAttribute('style') || 'default').toLowerCase()
            return {
              text: b.textContent || 'Button',
              style: (styles.includes(raw) ? raw : 'default') as ButtonStyle,
            }
          })
          return { buttons }
        },
  ...
  // Inline button — an inline-formatting ATOM (like custom_emoji): a small
  // pill flowing inside text. Official form <tg-button type="…" style="…">.
  inline_button: {
    attrs: {
      text: { default: 'Button' },
      style: { default: 'default' }, // ButtonStyle
      type: { default: 'url' }, // ButtonType
      url: { default: '' },
      data: { default: '' },
      query: { default: '' },
      copyText: { default: '' },
    ...
    parseDOM: [
      {
        ...
        getAttrs: (el) => {
          const dom = el as HTMLElement
          return {
            text: dom.textContent || 'Button',
            style: dom.getAttribute('style') || 'default',
            type: dom.getAttribute('type') || 'url',
            url: dom.getAttribute('url') || '',
            data: dom.getAttribute('data') || '',
            query: dom.getAttribute('query') || '',
            copyText: dom.getAttribute('text') || '',
          }
        },
      },
    ],
    toDOM: (node) => [
      'tg-button',
      {
        ...
        style: node.attrs.style === 'default' ? undefined : node.attrs.style,
        url: node.attrs.url || undefined,
        data: node.attrs.data || undefined,
        query: node.attrs.query || undefined,
        text: node.attrs.copyText || undefined,
      },
      node.attrs.text,
    ],
  },
