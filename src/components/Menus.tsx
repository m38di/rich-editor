  ...
  return (
    <Sheet onClose={close} title="Table">
      <div className="sheet-card">
        ...
        <Row icon={<Iv name="table" size={16} />} iconBg="#5856D6" label="Compact" active={ctx.attrs.compact} keep onClick={() => act(toggleCompact, false)} />
...
export function AttachSheet({ run, close, onOpenTableSize }: AttachSheetProps) {
  const items: { icon: string; bg: string; label: string; onClick: () => void }[] = [
    ...
    { icon: 'document', bg: '#64D2FF', label: 'Document', onClick: () => run(insertMedia('document')) },
    ...
    { icon: 'button', bg: '#5AC8FA', label: 'Buttons', onClick: () => run(insertButtonRow) },
  ]
  return (
